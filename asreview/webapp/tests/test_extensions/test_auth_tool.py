...
# @pytest.fixture(scope="class")
# def no_auth(request, client_no_auth):
#     """This fixture starts a non-authenticated version of ASReview"""
#     request.cls.client = client_no_auth
#     yield request.cls


def test_existence_empty_test_folder(no_auth):
    # check existence folder
    assert asreview_path().exists() is True
    # check if folder is empty
    assert list(asreview_path().glob("*")) == []


# test that ensures we can create 2 projects
def test_create_project(no_auth):
    client = no_auth
    sc1, data1 = au.create_project_from_dict(client, PROJECTS[0])
    sc2, data2 = au.create_project_from_dict(client, PROJECTS[1])
    assert (sc1, sc2) == (201, 201)
    # check if asreview folder contain 2 projects
    assert len(list(asreview_path().glob("*"))) == 2
    # check if projects are there
    folders = set([f.name for f in asreview_path().glob("*")])
    for d in [data1, data2]:
        assert bool(d.get("id"))
        assert d.get("id") in folders
...
class TestNoAuthentication:
    ...
    # initial test, make sure we always start with an empty ASReview folder


    # test that ensures we can create 2 projects
    # def test_creating_2_projects(self, client_no_auth):
    #     print(client_no_auth)
    #     assert True
