...
# Test user must be signed in to get profile
def test_signed_in_to_get_profile(client_auth):
    (status_code, data) = au.get_profile(client_auth)
    # asserts
    assert status_code == 401
    assert data["message"] == "Login required."
...
# Test user must be signed in to update profile
def test_signed_in_to_update_profile(client_auth):
    (status_code, data) = au.update_user(client_auth, {})
    # asserts
    assert status_code == 401
    assert data["message"] == "Login required."
