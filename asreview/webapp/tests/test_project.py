...
link_project_file = "https://github.com/asreview/asreview-project-files-testing/tree/master/asreview_file"
...
def test_import_project(tmp_path, client, url):
    ...
    assert any(item["id"] == f"{json_data_import['id']}" for item in json_data_projects["result"])

    # Test get progress info on the article
    response_progress = client.get(f"/api/project/{json_data_import['id']}/progress")
    ...
    response_progress_history = client.get(f"/api/project/{json_data_import['id']}/progress_history")
    ...
    response_progress_efficiency = client.get(f"/api/project/{json_data_import['id']}/progress_efficiency")
    ...
    response_get_document = client.get(f"/api/project/{json_data_import['id']}/get_document")
    ...
    response_classify_instance = client.post(f"/api/project/{json_data_import['id']}/record/<doc_id>", data={
        "doc_id": 99,
        "label": 1,
    })
    ...
    response_update_classify = client.put(f"/api/project/{json_data_import['id']}/record/<doc_id>", data={
        "doc_id": 99,
        "label": 0,
    })
    ...
    response_prior = client.get(f"/api/project/{json_data_import['id']}/prior")
    json_data_prior = response_prior.get_json()
    assert "result" in json_data_prior
    assert isinstance(json_data_prior["result"], list)

    # Test export result
    response_export_result_excel = client.get(f"/api/project/{json_data_import['id']}/export?file_type=excel")
    response_export_result_csv = client.get(f"/api/project/{json_data_import['id']}/export?file_type=csv")
    response_export_result_tsv = client.get(f"/api/project/{json_data_import['id']}/export?file_type=tsv")
    ...
    # Test export project
    response_export_project = client.get(f"/api/project/{json_data_import['id']}/export_project")
    assert response_export_project.status_code == 200

    # Test finish project
    response_finish = client.get(f"/api/project/{json_data_import['id']}/finish")
    assert response_finish.status_code == 200

    # Test delete project
    response_delete = client.delete(f"/api/project/{json_data_import['id']}/delete")
