...
import { Card, CardContent, Grid, Stack, Box, Skeleton, IconButton, Typography, FormControlLabel, Switch, useTheme } from "@mui/material";
...
const sortPerChunk = (decisions, chunkSize) => {
  ...
  for (let i = 0; i < decisions.length; i += chunkSize) {
    const chunk = decisions.slice(i, i + chunkSize).sort((a, b) => b.Label - a.Label);
    sorted.push(...chunk);
  }
  return sorted;
};
...
const generateLines = (total, decisions, chronological, chunkSize) => {
  const lines = [];
  const sortedDecisions = chronological ? decisions : sortPerChunk(decisions, chunkSize);

  for (let i = 0; i < total; i++) {
    if (i < sortedDecisions.length) {
      lines.push(
        <HistoryItem key={i} color={
          sortedDecisions[i].Label === 1 ? '#FFD700' :
          sortedDecisions[i].Label === 0 ? '#808080' :
          '#D3D3D3'
        } />
      );
    } else {
...
const HistoryBar = ({ labelingChronologyQuery, progressQuery, mobileScreen }) => {
  ...
  const updateChunkSize = useCallback(() => {
    if (containerRef.current) {
      ...
      const itemWidth = parseFloat(theme.spacing(3.15)) + parseFloat(theme.spacing(0.25)); // item width + margin
              ...
              <Stack spacing={1} alignItems="center" width="100%">
                <Box display="flex" justifyContent="space-between" alignItems="center" width="100%">
                  <Box display="flex" alignItems="center">
                    <Typography variant="h6" fontSize={!mobileScreen ? '1.25rem' : '1rem'}>
                      Labeling History
                    </Typography>
                  ...
                  <Box width="100%">
                    <Stack direction="row" flexWrap="wrap" justifyContent="flex-end">
                      {Array.from(new Array(30)).map((_, index) => (
                        <Skeleton key={index} variant="rectangular" width={theme.spacing(3.15)} height={theme.spacing(0.625)} style={{ margin: theme.spacing(0.125) }} />
                      ))}
                    </Stack>
                  </Box>
                ) : (
                  <HistoryContainer expanded={expanded ? 1 : 0} ref={containerRef}>
                    {generateLines(visibleCount, latestDecisions, chronological, chunkSize)}
                  </HistoryContainer>
                )}
              </Stack>
            </Grid>
