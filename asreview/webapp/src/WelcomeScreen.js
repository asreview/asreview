...
const WelcomeScreen = (props) => {
  ...
  const [bootStatus, setBootStatus] = useState('asreview');

  useEffect(() => {
    const fetchData = async () => {
      await axios.get(api_url + "boot")
          .then(result => {
            console.log('Boot status: ' + result.data['status']);
            setBootStatus(result.data['status']);
            setLoad(true);
          ...
          >
            <Box className={classes.background}>
              <Box className={classes.root}>
                <div className={classes.logo}  className={classes.svgFill}>
