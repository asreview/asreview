...
class ModelChangeSampling(QueryStrategy):
    ...
    def __init__(self, pool, method='lcbmc', model=None, prev_score=None):
        ...
        if self.method not in ['lcbmc']:
            raise TypeError("supported methods are ['lcbmc'], the given one "
                            "is: " + self.method)

    def make_lcbmc(self, pred_vals):
        qs = UncertaintySampling(self._pool, method='lcb', model=self.model)
    ...
    def make_query(self, n=1):
        ...
        unlabeled_entry_ids, X_pool = zip(*self._pool.get_unlabeled_entries())
