...
| DOI      | doi                                                                                                     | no        |
+----------+---------------------------------------------------------------------------------------------------------+-----------+
| Priors_1 | final_included, label, label_included, included_label, included_final, included, included_flag, include | no        |
+----------+---------------------------------------------------------------------------------------------------------+-----------+
| Priors_2 | debug_label                                                                                             | no        |
+----------+---------------------------------------------------------------------------------------------------------+-----------+

\* Only a title or an abstract is mandatory.

**ID**   If your data contains a column titled ``record_id`` it needs to
consists only of intigers, and it should contain no missing data and no
...
**DOI**
If adigital identifier ( ``DOI``) is available it will be displayed during the
...
**Priors_1** If a column is available indicating previous labeling decisions
it should be binary with ``0`` = irrelevant/excluded, and ``1`` =
relevant/included. It can be used in the :doc:`ASReview command line interface
for simulations<../API/cli/>`, and then labes are required for ALL records. In
ASReview LAB, if labels are available for a part of the dataset (see
:doc:`partly labeled data <../features/pre_screening>`), the labels will be
automatically detected and used for prior knowledge. The first iteration of
the model will then be based on these decissions and used to predict relevance
scores for the unlabelled part of the data.

**Priors_2** You can explore a previously labeled dataset in the Exploraton
Mode. A column called ``debug_label`` is then needed, indicating the relevant
and irrelevant records with ones and zeroes. The relevant records will show up
green during screening. This option is usefull for training purposes or for
